...
public abstract class Functions{
	...
	public static String getFileWithoutPrefix(String filename, String prefix){
		if(filename.indexOf(prefix) > -1){
			String fileNameWithoutPrefix = filename.substring(filename.indexOf(prefix));
			fileNameWithoutPrefix.trim();
			return fileNameWithoutPrefix;
		}
		System.out.println("Prefix "+ prefix +" not found in "+ filename);
		return filename;
	}
	...
	for (int i=0; i<len; i++)
		...
		if (args[i].charAt(0) == '-' && args[i].charAt(1) == '-')
			...
			val = new String();
			while (i+1 < args.length && !(args[i+1].charAt(0) == '-' && args[i+1].charAt(1) == '-' ))
			    {
				val += args[i+1] + " ";
				i++;
			    }
	...
	public static void chopUpSequence(String fileName){
		try{
			
			ExtendedReader PTTFileReader = new ExtendedReader(new FileReader(fileName));
			ExtendedWriter newFile = new ExtendedWriter(new FileWriter(fileName+"_choppedUp"));
			PTTFileReader.more();
...
//	
//	public static float findMax(ArrayList<Float> unsortedScores){
//		float max = unsortedScores.get(0);
//		for(int i = 1; i < unsortedScores.size();i++){
//			if(unsortedScores.get(i) > max)
//				max = unsortedScores.get(i);
...
//		return max;
//	}
//	public static int findMax(ArrayList<Integer> unsortedScores){
//		int max = unsortedScores.get(0);
//		for(int i = 1; i < unsortedScores.size();i++){
//			if(unsortedScores.get(i) > max)
//				max = unsortedScores.get(i);
...
//
//	
//	public static float findMin(ArrayList<Float> unsortedScores){
//		float min = unsortedScores.get(0);
//		for(int i = 1; i < unsortedScores.size();i++){
//			if(unsortedScores.get(i) < min)
//				min = unsortedScores.get(i);
...
//	}
//	
//	public static int findMin(ArrayList<Integer> unsortedScores){
//		int min = unsortedScores.get(0);
//		for(int i = 1; i < unsortedScores.size();i++){
//			if(unsortedScores.get(i) < min)
//				min = unsortedScores.get(i);
